from app.adapters.base import RawJobPayload
from app.adapters.common import (
    as_mapping,
    join_location_parts,
    normalize_employment_type,
    normalize_remote_type,
    object_list,
    parse_date,
    parse_json_object,
    parse_salary_mapping,
    payload_hash,
    split_locations,
    text,
)
from app.adapters.schema import RawJobPosting


class SmartRecruitersAdapter:
    source = "smartrecruiters"
    extraction_method = "official_api_fixture"

    def parse(self, payload: RawJobPayload, *, source_url: str) -> list[RawJobPosting]:
        data = parse_json_object(payload)
        digest = payload_hash(payload)
        postings: list[RawJobPosting] = []
        for job in object_list(data.get("content")):
            company = as_mapping(job.get("company"))
            location = as_mapping(job.get("location"))
            location_text = join_location_parts(
                location.get("city"),
                location.get("region"),
                location.get("country"),
            )
            locations = split_locations(location_text)
            salary_min, salary_max, salary_currency = parse_salary_mapping(job.get("salary"))
            postings.append(
                RawJobPosting(
                    source=self.source,
                    source_url=text(job.get("ref"), default=source_url),
                    application_url=text(job.get("applyUrl"), default=text(job.get("ref"))),
                    external_id=text(job.get("id")),
                    title=text(job.get("name")),
                    company=text(company.get("name")),
                    locations=locations,
                    remote_type=normalize_remote_type(job.get("remote"), locations=locations),
                    employment_type=normalize_employment_type(job.get("typeOfEmployment")),
                    salary_min=salary_min,
                    salary_max=salary_max,
                    salary_currency=salary_currency,
                    posted_date=parse_date(job.get("releasedDate")),
                    valid_through=parse_date(job.get("validThrough")),
                    extraction_method=self.extraction_method,
                    raw_payload_hash=digest,
                )
            )
        return postings
